...
| 項目 | 内容 |
|------|------|
| 初回 / 更新 | リポジトリルートで `npx gitnexus analyze . --skip-agents-md`（`--skip-agents-md` で既存のエージェント用 MD を上書きしない） |
| 生成物 | `.gitnexus/`（**`.gitignore` で除外**・コミット不要） |
| Cursor | [`.cursor/mcp.json`](.cursor/mcp.json) に `gitnexus` を定義済み。**設定変更後は Cursor を再起動**して MCP を読み込む |
| ライセンス | GitNexus パッケージは [PolyForm Noncommercial 1.0.0](https://polyformproject.org/licenses/noncommercial/1.0.0/)。**商用利用の可否は別途確認**すること |

...
- **バージョン:** `manifest.json` の `version` に従う（現行: `1.3.0`）
...
- `ChatMessage`: `role` は `system` | `user` | `assistant`、`content` はプレーンテキスト。任意で **`createdAt`**（ISO 8601 UTC、Vault / UI の時系列用）。**API へは `role` と `content` のみ**渡す（[`ChatSession` の `toApiMessages`](../src/core/chat-session.ts)）。
...
- **`ChatSessionDelegate`:** UI 更新用コールバック。`onSendStarting(userContent, userAt)` で仮 DOM 行（`userAt` は ISO UTC、ユーザ行の時刻表示に使用）、`onStreamChunk` でプレーン累積、**ストリーム完了後・追記前**に `onStreamFinished`（アシスタントの Markdown 確定描画）、追記成功後に `onTurnComplete(userContent, result, assistantAt)`（usage 行・入力クリア・助手時刻）、失敗・中止時に `onTurnRolledBack`、読み込み表示に `onLoadingChanged`、Truncate 後に `onMessagesChanged`、セッションクリア時に `onSessionCleared`、トークン超過時に `promptTokenLimitChoice`（Modal）、Notice に `showNotice`。
...
- 各ブロックの先頭（本文の前）に、任意で 1 行 **`<!-- ai-chat-at:<ISO8601 UTC> -->`** を置ける。パーサはこれを `createdAt` にし、**`content` には含めない**（ADR 0003）。
...
2. **履歴**（`ai-chat-history`）— 縦フレックス。各メッセージは `ai-chat-msg` + ロール修飾子 `ai-chat-msg-user`（右寄せ）または `ai-chat-msg-assistant`（左寄せ）。ロールラベル（`ai-chat-msg-role`）の下に、**`createdAt` があるときのみ**ローカル時刻短文（`ai-chat-msg-time`）。その下に **吹き出し内側** `ai-chat-msg-bubble-inner`（`overflow-x: auto`、`word-break` 系でコードブロック・テーブルが横幅を突き破らない）。確定済み本文は `MarkdownRenderer.render`、ストリーム中のアシスタントのみプレーンテキスト → 完了後に再描画。
...
8. **追記成功後のみ:** `session.messages` に user → assistant（`result.content`）を **`createdAt` 付きで** `push`、`onTurnComplete` で usage 行（`ai-chat-usage-meta`）・助手時刻（`ai-chat-msg-time`）を表示、入力クリア、`pendingStreamRows` をクリア。
...
`leadingSep = "\n\n"` と本文を連結し、**追記ブロック先頭は必ず 2 改行**で既存本文と視覚的に分離する。各ターンで **User / Assistant それぞれ**に `userAt` / `assistantAt`（`Date.toISOString()`）を取り、見出し直後に **`<!-- ai-chat-at:<ISO> -->`** を 1 行挿入する（詳細は ADR 0003）。全体を `vault.append` する。
...
主要クラス: `ai-chat-root`, `ai-chat-history`, `ai-chat-msg`, `ai-chat-msg-role`, `ai-chat-msg-time`, `ai-chat-input-row`, `ai-chat-input`, `ai-chat-toolbar`, `ai-chat-loading`, `ai-chat-target`、ストリーミング用 `ai-chat-md-stack`, `ai-chat-plain-layer`, `ai-chat-md-layer`, `ai-chat-reason-plain`, `ai-chat-usage-meta`。Obsidian CSS 変数（`--background-modifier-border` 等）に依存。
...
| 2026-04-06 | **v1.2.1** 確定: Android 手動サイドロード手順、Web Search 送信前確認を Obsidian Modal 化（`web-search-confirm-modal.ts`）、狭幅ツールバー CSS、Phase J ミッション・手動検証（`TEST_SPEC` §9） |  |
| 2026-04-06 | メッセージ時刻: `ChatMessage.createdAt`、Vault `<!-- ai-chat-at -->`、UI `ai-chat-msg-time`、ADR 0003 |  |
| 2026-04-06 | **v1.3.0** 確定: `manifest.json` / `package.json` を `1.3.0` に。上記 v1.2.1 相当（Phase J Android）・メッセージ時刻（Phase K）・ツールバー `mod-cta` レイアウト修正を含むタグ付きリリース | `c90761a5240448bca535e4445b956f4a7463766c` |
